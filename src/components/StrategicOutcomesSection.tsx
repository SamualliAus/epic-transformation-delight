...
const StrategicOutcomesSection: React.FC = () => {
  ...
  return <section ref={sectionRef} id="strategic-outcomes" className="py-20 bg-white">
      <div className="container-custom">
        <div className="max-w-3xl mx-auto mb-12 text-center">
          <h2 className={cn("mb-4 opacity-0", isVisible && "animate-fade-in")}>
            What Makes EPiC Different
          </h2>
          <p className={cn("text-xl text-gray-700 mb-10 opacity-0", isVisible && "animate-fade-in")} style={{
          animationDelay: '0.2s'
        }}>We don't do transformational consulting. We unlock progress - through 9 powerful Accelerators tied to the outcomes that matter most:</p>
        </div>

        <div className="grid md:grid-cols-3 gap-6">
          {outcomes.map((outcome, index) => <Card key={index} className={cn("transition-all duration-500 hover:transform hover:translate-y-[-5px] border-t-4 hover:shadow-xl opacity-0 cursor-default", getColorClasses(outcome.color), isVisible && "animate-fade-in-right")} style={{
          animationDelay: `${0.2 * index}s`
              ...
              <div className="p-6">
                ...
                <div className="flex flex-col space-y-1">
                  {outcome.accelerators.map((accelerator, accIndex) => <button key={accIndex} className="text-left text-sm font-medium text-epic-blue hover:text-epic-orange transition-colors flex items-center" onClick={() => scrollToAccelerator(accelerator)}>
