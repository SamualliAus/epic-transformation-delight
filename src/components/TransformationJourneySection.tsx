...
const TransformationJourneySection: React.FC = () => {
    ...
    const handleScroll = () => {
      if (sectionRef.current) {
        ...
        if (sectionTop <= viewportHeight && sectionTop > -sectionHeight) {
          const progress = Math.min(1, Math.max(0, (viewportHeight - sectionTop) / (viewportHeight + sectionHeight - 200)));
  ...
  return <section ref={sectionRef} id="transformation-journey" className="py-20 bg-gradient-to-br from-gray-50 to-white relative overflow-hidden">
      ...
      <div className="container-custom">
        <div className="max-w-3xl mx-auto mb-12 text-center">
          <h2 className={cn("mb-4 opacity-0", isVisible && "animate-fade-in")}>Our Unique Approach to Delight You.</h2>
          <p className={cn("text-xl text-gray-700 mb-12 opacity-0", isVisible && "animate-fade-in")} style={{
          animationDelay: '0.2s'
        }}>In our experience, we know that transformational change isn't one-size-fits-all and no two journey's are the same. We've identified four phases of change that enables us to meet you where you are, providing bite-sized problem-solving capabilities that really do move the needle.</p>
        </div>
        ...
        <div className="relative max-w-2xl mx-auto">
          ...
          <div className="space-y-16 overflow-x-auto pb-6">
            ...
            animationDelay: `${0.2 * index}s`
                ...
                <div className={cn("absolute left-0 top-0 flex items-center justify-center w-8 h-8 rounded-full bg-white border-2 border-gray-200 z-10 transition-transform duration-300", scrollProgress > index / journeySteps.length && "scale-125 border-epic-blue")}>
                  <div className={cn("opacity-0", scrollProgress > index / journeySteps.length && "animate-scale-in")}>
                    {step.icon}
                  </div>
                </div>
        ...
        <div className="lg:hidden mt-16 overflow-x-auto pb-6">
          <div className="flex space-x-6 min-w-max px-4">
            {journeySteps.map((step, index) => <div key={index} className={cn("relative w-72 p-6 border rounded-lg shadow-sm opacity-0", isVisible && "animate-fade-in")} style={{
            animationDelay: `${0.2 * index}s`
          }}>
